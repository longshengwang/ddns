...
public class ProxyServer extends Thread {
    ...
    public void closeSocketByIndexId(Integer indexId) throws IOException {
        ...
        lock.lock();
        try {
            LOG.info("ProxyLoopData需要关闭的indexid:" + indexId);
    ...
    public void closeSocketByKey(SelectionKey clientKey) throws IOException {
        LOG.info("[closeSocketByKey]>> 需要关闭client key:" + clientKey);
        lock.lock();
                ...
                while (selectionKeys.hasNext()) {
                    ...
                    try {
                        if (readyKey.isValid() && readyKey.isAcceptable()) {
                            ...
                            LOG.info("远程连接端口:" + socketChannel.getRemoteAddress());
                            ...
                            if(connModel.getSecurity().equals( RegisterProtocol.SECURITY )){
                                ...
                                if(!connModel.getTrustIps().contains(ip)){
                                    LOG.warn("该通道是安全通道，IP:" + ip + " 不在信任列表中，需要先认证！");
                                    socketChannel.close();
                                    continue;
                                }
                            }
                            ...
                            if (keyConvertMap.get(readyKey) == null) {
                                ...
                                LOG.info("===>>>>>从外面过来的indexID:"+indexId);
                                if (indexId == null) {
...
//        System.out.println(indexQueue.poll());
        ...
        String b = new String("a1aa");
        xx.add("aaa");
        xx.add("aa1");
        xx.add("aa2");
        System.out.println(xx.contains(b));
    }
}
